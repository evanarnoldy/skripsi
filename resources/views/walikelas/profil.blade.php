        ...
        @endif
            ...
            <div class="row no-gutters">
                <div class="col-md-3">
                    ...
                    <form enctype="multipart/form-data" action="{{url('wali/profil-wali')}}" method="POST">
                        @csrf
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col">
                                ...
                                <p>: {{$user->NIP}}</p>
                                <p>: {{$user->email}}</p>
                                <p>: {{$user->kelas_diampu}}{{$user->unit}}</p>
                                <p>: {{$user->jenis_kelamin}}</p>
                                <p>: {{date('d-m-Y', strtotime($user->tanggal_lahir))}}</p>
                                <p>: {{$user->alamat}}</p>
                            </div>
                        </div>
                        <a href="{{url('wali/profil-wali/'.$user->id .'/edit-profil')}}" class="btn btn-primary">Edit profil</a>
                    </div>
                </div>
            </div>
